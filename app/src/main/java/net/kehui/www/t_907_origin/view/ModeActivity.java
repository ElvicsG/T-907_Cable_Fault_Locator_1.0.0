...
//                            tdrAutoTestLong();
    ...
    private void calculateDistance(int cursorDistance) {
        ...
        if (mode == DECAY) {
          ...
          if ((leadLength > 0) && (MainActivity.leadCat == 1)) {  //jk20201130  脉冲电流延长线不选就不计算//if (leadLength > 0) {
                //实际点数
    ...
    void  balanceAutoTdr(){
        ...
        for (int i = 0; i <= j; i++) {
          /*  if ( waveArray[i] < 132) {    //取128
            ...
            if ( waveArray[i] < Median_value) {    //取128 //jk20210520
...
//        for (i = start + 64; i < start + 72; i++) {
    ...
    private void calculatePulse() {
        ...
        //有测试缆  //GC20200103
        Log.e("leadCat", "leadCat" +MainActivity.leadCat);
        if ((leadLength > 0) && (MainActivity.leadCat == 1)) {  //if (leadLength > 0) {// if ((leadLength > 0) && (leadCat == 1)) {  //jk20201130  脉冲电流延长线不选就不计算
            z = (double)50;
        } else {
            z = (double)25;
        }
        ...
        if (mode == TDR) {
            ...
            tDialog = new TDialog.Builder(getSupportFragmentManager())
                    ...
                    .setOnKeyListener(new DialogInterface.OnKeyListener() {
                      //  @OverriderangeSave //jk20220831 编译失败
                      @Override
